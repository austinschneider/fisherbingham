...
def expm_Higham08(A, balancing=True):
    ...
    if balancing:
        baP_B, baP_scale, _ = scipy.linalg.matrix_balance(A, separate=True, scale=False, permute=True)
        baP = balance(A,"P")# -> error for non-classical matrix  -- "FIXME": balance()
        baS_B, baS_scale, _ = scipy.linalg.matrix_balance(A, separate=True, scale=True, permute=False)
        A = np.copy(baS_B)
